...
let uiTimer;
for(const dir of ['components','styles','lib','trou-noir'])watch(resolve(root,dir),{recursive:true},(_,file)=>{if(!file||! /\.(tsx?|css)$/.test(file)||file==='utilities.css')return;clearTimeout(uiTimer);uiTimer=setTimeout(()=>buildBlackHole().then(()=>console.log('React island rebuilt. Refresh your browser.')).catch(console.error),300);});
