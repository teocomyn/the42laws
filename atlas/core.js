(function(root){
 ...
 function cleanState(value){
  ...
  return {version:1,read:ids(data.read),saved:ids(data.saved),notes,labs:Array.isArray(data.labs)?[...new Set(data.labs.filter(x=>['neutrino','antimatiere','photon','temps','navier-stokes','relativite','trou-noir'].includes(x)))]:[],last:Number.isInteger(data.last)&&data.last>=1&&data.last<=42?data.last:null};
 }
