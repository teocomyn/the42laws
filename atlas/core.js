(function(root){
 ...
 function cleanState(value){
  ...
  return {version:1,read:ids(data.read),saved:ids(data.saved),notes,labs:Array.isArray(data.labs)?[...new Set(data.labs.filter(x=>['neutrino','antimatiere','photon','temps','navier-stokes'].includes(x)))]:[],last:Number.isInteger(data.last)&&data.last>=1&&data.last<=42?data.last:null};
 }
 ...
 function parseNotebookJSON(text){
  ...
  if(!Array.isArray(d.labs)||d.labs.some(x=>!['neutrino','antimatiere','photon','temps','navier-stokes'].includes(x)))throw new Error('Liste des laboratoires invalide.');
