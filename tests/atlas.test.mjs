...
test('atlas preserves all 42 questions, seven domains and current research states',()=>{assert.deepEqual(D.questions.map(q=>q.id),Array.from({length:42},(_,i)=>i+1));assert.equal(D.domains.length,7);assert.deepEqual(D.questions.filter(q=>q.researched).map(q=>q.id),[1,10,15,19,23,25,26,30,32,33,34,35,36,41]);assert.equal(D.questions[40].sourceCount,63);for(const q of D.questions){assert.ok(q.title);assert.ok(D.domains.some(d=>d.id===q.domain&&q.id>=d.range[0]&&q.id<=d.range[1]));}});
...
test('every required path step has a real synthesis and every synthesis has reading landmarks',()=>{
 ...
 for(const id of [1,19,25,26,30,32,33,34,35,36]){
